...
#ifndef SCALE_BUFFER_HPP
#define SCALE_BUFFER_HPP
...
namespace scale {
  ...
  class Buffer : public boost::equality_comparable<Buffer>,
                 public boost::equality_comparable<MutSpanOfBytes>,
   ...
   public:
     ...
     */
    explicit Buffer(std::vector<uint8_t> vector);
    explicit Buffer(const RangeOfBytes auto &range);
    ...
    /**
     * @brief Lexicographical comparison of buffer content and range of bytes
     */
    bool operator==(const RangeOfBytes auto &other) const noexcept;
     ...
     * @return this buffer, suitable for chaining.
     */
    Buffer &put(const RangeOfBytes auto &range);
    ...
    [[deprecated("Use Buffer::put() instead that")]] Buffer &putBuffer(
        const Buffer &buf);
...
namespace std {
  template <>
  struct hash<scale::Buffer> {
    size_t operator()(const scale::Buffer &x) const {
      return boost::hash_range(x.begin(), x.end());
    }
  };
}  // namespace std

#endif  // SCALE_BUFFER_HPP
